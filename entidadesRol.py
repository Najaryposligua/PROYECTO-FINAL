...
class Empleado(ABC):
    ...
    def mostrarEmpleado(self):
        print(' {} Empleado : {} Cedula: {} dirección : {} Cargo: {} Dpto: {}'.format(self.id, self.nombre, self.cedula,
                                                                                      self.direccion,
...
class Administrativo(Empleado):
    def __init__(self, nombre, departamento, cargo, direccion, cedula, telefono, fechaIngreso, sueldo, id,
                 comision=True):
        super().__init__(nombre, departamento, cargo, direccion, cedula, telefono, fechaIngreso, sueldo, id)
        self.comision = comision

    def mostrarEmpleado(self):
        print(' {} Administrativo : {} Cedula: {} dirección : {} Cargo: {} Dpto: {}'.format(self.id, self.nombre,
                                                                                            self.cedula, self.direccion,
                                                                                            self.cargo.descripcion,
                                                                                            self.departamento.descripcion),
              end=" ")
    ...
    def getEmpleado(self):
        return [self.id, self.nombre, str(self.departamento.id), str(self.cargo.id), self.direccion, self.cedula,
...
class Obrero(Empleado):
    ...
    def mostrarEmpleado(self):
        print(' {} Obrero : {} Cedula: {} dirección : {} Cargo: {} Dpto: {}'.format(self.id, self.nombre, self.cedula,
                                                                                    self.direccion,
                                                                                    self.cargo.descripcion,
                                                                                    self.departamento.descripcion),
              end=" ")
    ...
    def getEmpleado(self):
        return [self.id, self.nombre, str(self.departamento.id), str(self.cargo.id), self.direccion, self.cedula,
                self.telefono, str(self.fechaIngreso), str(self.sueldo), str(self.cc)]
...
class Deduccion:
    ...
    def mostrarDeduccion(self):
        print('Valor Iess {} = \n Valor comision ({}) = \n Valor antiguedad ({}) ='.format(self.iess, self.comision,
                                                                                           self.antiguedad))
...
class Prestamo:
    ...
    def mostrarPrestamo(self):
        print('''{}° Prestamo realizado: {}
          ...
          - Cuota = ${:.2f} 
          - Saldo = ${:.2f}
          - estado = {} '''.format(self.id, self.aamm, self.empleado.nombre, self.valor, self.numPagos, self.cuota,
                                   self.saldo, self.estado))

    def getPrestamo(self):
        return [str(self.id), self.empleado.id, self.aamm, str(self.valor), str(self.numPagos), str(self.cuota),
                str(self.saldo), str(self.estado)]
...
class Sobretiempo:
    ...
    def mostrarSobretiempo(self):
        print('''{}° Sobretiempo realizado: {}
          ...
          - H100 = {}  
          - Valor = ${:.2f} 
          - estado = {} '''.format(self.id, self.aamm, self.empleado.nombre, self.h50, self.h100, self.sobretiempo(),
                                   self.estado))
...
class Nomina:
    ...
    def calcularNominaDetalle(self, empleado, deduccion):
        ...
        self.detalleNomina.append([
            ...
            str(rubrosIngresos[0]), str(rubrosIngresos[1]), str(rubrosIngresos[2]), str(rubrosIngresos[3]),
            str(rubrosIngresos[4]),
    ...
    def mostrarDetalleNominaVertical(self, empleado):
        gotoxy(10, 9);print('FECHA: {}  '.format(self.fecha))
        gotoxy(10, 10);print('Nomina correspondiente al Periodo:{}'.format(self.aamm))
        gotoxy(10, 11);print('De : {}'.format(empleado.nombre))
        gotoxy(10, 12);print('Direccion empleado : {}'.format(empleado.direccion))
        gotoxy(10, 12);print('Cedula empleado : {}'.format(empleado.cedula))
        gotoxy(10, 14);print('Telefono empleado : {}'.format(empleado.telefono))
        gotoxy(10, 15);print('Fecha de Ingreso empleado : {}'.format(empleado.fechaIngreso))
        gotoxy(10, 16);print('Sueldo empleado : {}'.format(empleado.sueldo))
        ...
        for det in self.detalleNomina:
            ...
            gotoxy(10, 17);print("----------------------------------------------------------------------------- ", end="")
            gotoxy(10, 18);print("Nombre : ",det[0], end="")
            gotoxy(10, 19);print("Cargo : ",desCargo, end="")
            gotoxy(10, 20);print("Departamento : ",desDpto, end="")
            gotoxy(10, 21);print("Sueldo : ",det[3], end="")
            gotoxy(10, 22);print("Sobretiempo : ",det[4], end="")
            gotoxy(10, 23);print("Antiguedad : ",det[5], end="")
            gotoxy(10, 24);print("Comision : ",det[6], end="")
            gotoxy(10, 25);print("TotIng : ",det[7], end="")
            gotoxy(10, 26);print("Iess : ",det[8], end="")
            gotoxy(10, 27);print("Prestamo : ",det[9], end="")
            gotoxy(10, 28);print("TotDes : ",det[10], end="")
            gotoxy(10, 29);print("Neto : ",det[11], end="")
...
class DetalleNomima(CalculoRol):
    ...
    def getSobretiempo(self, aamm):
        ...
        if self.empleado.id[0] == "O":
            ...
            sobretiempo = archiSobretiempo.buscar2(self.empleado.id, aamm)
            if sobretiempo:
                entSobretiempo = Sobretiempo(self.empleado, sobretiempo[2], float(sobretiempo[3]), int(sobretiempo[4]),
                                       bool(sobretiempo[5]), sobretiempo[0])
                return entSobretiempo.sobretiempo()
            else:
                return 0
    ...
    def getAntiguedad(self, deduccion):
        ...
        if self.empleado.id[0] == "O":
            ...
            return round(self.empleado.sueldo * deduccion.getAntiguedad(), 2)
        return calAnt
